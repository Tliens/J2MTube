...
enum J2MCodeBuilderCodeType: Int {
    case OC = 1
    case Swift
    case Dart
    case TypeScript
...
enum J2MCodeBuilderJSONModelType: Int {
    case None = 0
    case YYModel
    case MJExtension
    case HandyJSON
...
class J2MCodeBuilder: NSObject {
    
    var config = J2MCodeBuilderConfig()
    ...
    func generateCode(with jsonObj:Any, complete:BuildComplete?){
        ...
        //  Copyright © \(year) J2MTube. All rights reserved.
        //\n
        """
    ...
    func generateFile(with filePath:String?, hString:NSMutableString, mString:NSMutableString, complete:GenerateFileComplete?) {
        if hString.length > 0 && mString.length > 0 {
            ...
            var success = false
            if filePath == nil {
                if let deJ2MtopPath = NSSearchPathForDirectoriesInDomains(.desktopDirectory, .userDomainMask, false).last {
                    let path = deJ2MtopPath.appending("/J2MTubeFiles")
                    print("path = \(path)")
...
class J2MCodeBuilderConfig: NSObject {
    var superClassName = "NSObject"
    var rootModelName = "NSRootModel"
    var modelNamePrefix = ""
    var authorName = "J2MTube"
    var codeType: J2MCodeBuilderCodeType = .OC
    var jsonType: J2MCodeBuilderJSONModelType = .None
    var swiftType:J2MSwiftType = .class
